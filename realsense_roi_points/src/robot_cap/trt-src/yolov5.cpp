...
void Yolov5::yolo_ROI(cv::Mat inputImage, std::vector<Yolo::Detection> Input,
                      cv::Mat &color_ROI, cv::Mat &depth_ROI, cv::Mat black_image,
                      ...
                      std::vector<cv::Point2d> &out_Center)
{
    ...
    for (size_t j = 0; j < Input.size(); j++)
    {
        ...
        if (r.width > 0 && r.height > 0)
        {
            ...
            // 创建一个与原图像大小相同的黑色图像
            cv::Mat black_image = cv::Mat::zeros(inputImage.size(), inputImage.type());

            // 将原图中的ROI区域复制到黑色图像中
            inputImage(r).copyTo(black_image(r));
            // cv::imwrite("black_image.png", black_image);
        }
    }
}
