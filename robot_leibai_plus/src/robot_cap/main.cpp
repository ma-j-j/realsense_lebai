...
int main(int argc, char **argv)
{
    ...
    cv::Mat black_color_image, black_depth_image;
    cv::Mat color_black_image, depth_black_image;
    ...
    // 读模型
    // std::string engine_name = "src/robot_cap/weight/Citrus.engine";
    ...
    while (ros::ok())
    {
        ...
        // 出图
        YL.yolo_ROI(color_image, res_left,
                    color_ROI, depth_ROI,
                    black_color_image, black_depth_image,
                    depth_image, depth_masked,
                    ROI_Center);
        ...
        cv::Mat nearest_color_ROI, nearest_depth_ROI;

        // 遍历所有ROI
        for (size_t i = 0; i < ROI_Center.size(); i++)
        {
            // 提取当前ROI的像素坐标
            float pixe_center[2] = {ROI_Center[i].x, ROI_Center[i].y};

            // 获取深度值
            float depth_value = aligned_depth_stream.get_distance(pixe_center[0], pixe_center[1]);

            // 确保深度值有效
            if (depth_value > 0 && depth_value < min_distance)
            {
                min_distance = depth_value;

                // 计算对应的矩形区域
                cv::Rect r = get_rect(color_image, res_left[i].bbox);
                r = r & cv::Rect(0, 0, depth_image.cols, depth_image.rows); // 边界检测

                // 创建一个与原图像大小相同的黑色图像
                color_black_image = cv::Mat::zeros(color_image.size(), color_image.type());
                depth_black_image = cv::Mat::zeros(depth_image.size(), depth_image.type());

                // 将原图中的ROI区域复制到黑色图像中
                color_image(r).copyTo(color_black_image(r));
                depth_image(r).copyTo(depth_black_image(r));

                // 复制颜色和深度ROI
                nearest_color_ROI = color_image(r).clone();
                nearest_depth_ROI = depth_image(r).clone();
                ...
                if (res_left.size() == 0)
                {
                    point_in_color_coordinates[0] = 0.0;
                    point_in_color_coordinates[1] = 0.0;
                    point_in_color_coordinates[2] = 0.1;
                }
                // 单位米
        ...
        /**************************ROI区域发布*******************************************/
        sensor_msgs::ImagePtr Color_ROI_Img_msgs = cv_bridge::CvImage(std_msgs::Header(), "bgr8", color_black_image).toImageMsg();
        sensor_msgs::ImagePtr depth_ROI_Img_msgs = cv_bridge::CvImage(std_msgs::Header(), "mono16", depth_black_image).toImageMsg();
